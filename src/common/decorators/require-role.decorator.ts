 ...
 *
 * Accepts `DefaultRole` (ADMIN) too, không chỉ `BusinessRole` - retryProductionOrder() (đợt kẹt
 * "SKU đã duyệt nhưng lệnh sản xuất tạo thất bại") là sự cố kỹ thuật cần ADMIN xử lý, không phải
 * quyết định nghiệp vụ của BOSS. `AuthenticatedUser.roles` là `string[]` thô ở runtime nên so
 * khớp đúng bất kể type gốc.
 */
